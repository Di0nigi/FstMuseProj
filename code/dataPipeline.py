...
def normalizeDim(dim,arr):
    ...
    if dim[0]==arr.shape[0]:
        ...
        elif arr.shape[0]<dim[0]:
            c=np.abs(dim[0]-arr.shape[0])
            #c=np.abs(arr.shape[0]-dim[0])
            arrM=tf.pad(arr, paddings=[[0, c], [0, 0]])
            #print(arr.shape)
            #arrM=arr
        return arrM
    
def normalizeDim1(dim,arr):
    if dim[1]==arr.shape[0]:
        return arr
    else:
        if arr.shape[0]>dim[1]:
            c=np.abs(arr.shape[0]-dim[1])
            arrM= arr[:-c, :]
        return arrM
    


def normalizeDim2(dim,arr):
    if dim[1]==arr.shape[1]:
        return arr
    else:
        if arr.shape[1]>dim[1]:
            c=np.abs(arr.shape[1]-dim[1])
            arrM= arr[0][:-c, :]
            arrM = tf.expand_dims(arrM, axis=0)
        return arrM
    
def normalizeShape(array, shape):
   
    original_shape = array.shape

    # Calculate the scaling factors for each dimension
    scale_factors = (np.array(shape) / np.array(original_shape)).tolist()

    # Use scipy.ndimage.zoom to resize the array
    normalized_array = zoom(array, scale_factors, order=1)  # Order=1 for bilinear interpolation
...
def onlySpect(trackList):
    spects=[]
    for elem in trackList:
        spects.append(elem.mSpGram)
...
if __name__ == '__main__':
    ...
    def main():
        dirPath="D:\dionigi\Documents\Python scripts\DeepLearning\\fstData\\toyData"
